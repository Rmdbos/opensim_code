...
ax.set_xlabel('X-displacement [m]', fontsize=12)
ax.set_ylabel('Y-displacement[m]', fontsize=12)
ax.set_zlabel('Z-displacement[m]', fontsize=12)
...
smallest_eigenvec_ind_c = np.argmin(eigenval)
largest_eigenvec = eigenvec[:, largest_eigenvec_ind_c]
smallest_eigenvec = eigenvec[:, smallest_eigenvec_ind_c]
largest_eigenval = eigenval[largest_eigenvec_ind_c]
smallest_eigenval = eigenval[smallest_eigenvec_ind_c]

if largest_eigenvec_ind_c == 0 and smallest_eigenvec_ind_c == 1:
    middle_eigenval = np.max(eigenval[2])
    middel_eigenvec = eigenvec[:,2]
elif largest_eigenvec_ind_c == 1 and smallest_eigenvec_ind_c == 0:
    middle_eigenval = np.max(eigenval[2])
    middel_eigenvec = eigenvec[:,2]
elif largest_eigenvec_ind_c == 0 and smallest_eigenvec_ind_c == 2:
    middle_eigenval = np.max(eigenval[1])
    middel_eigenvec = eigenvec[:,1]
elif largest_eigenvec_ind_c == 2 and smallest_eigenvec_ind_c == 0:
    middle_eigenval = np.max(eigenval[1])
    middel_eigenvec = eigenvec[:,1]
elif largest_eigenvec_ind_c == 1 and smallest_eigenvec_ind_c == 2:
    middle_eigenval = np.max(eigenval[0])
    middel_eigenvec = eigenvec[:,0]
elif largest_eigenvec_ind_c == 2 and smallest_eigenvec_ind_c == 1:
    middle_eigenval = np.max(eigenval[0])
    middel_eigenvec = eigenvec[:,0]
...
Tmat = np.column_stack([largest_eigenvec,middel_eigenvec,smallest_eigenvec])

beta = np.arcsin(Tmat[0,2])
gamma = np.arccos(Tmat[0,0]/np.cos(beta))
alpha = np.arccos(Tmat[2,2]/np.cos(beta))
print(alpha)

# alpha = np.arccos(largest_eigenvec[0])
# beta = np.arccos(largest_eigenvec[1])
# gamma = np.arccos(largest_eigenvec[2])

chisquare_val = 2.4477; 

a=chisquare_val*np.sqrt(largest_eigenval)
b=chisquare_val*np.sqrt(middle_eigenval)
c=chisquare_val*np.sqrt(smallest_eigenval)
...
def f_min(params, x, y, z):
    a1, b1, c1 = params
    return np.sum(ellipsoid(x, y, z, alpha, beta, gamma, a1, b1, c1)**2)
...
a = res.x[0]
b = res.x[1]
c = res.x[2]  # Semi-axes (adjust these for different ellipsoids)
...
ax.set_xlabel('X-stiffness[N/m] ', fontsize=12)
ax.set_ylabel('Y-stiffness[N/m]', fontsize=12)
ax.set_zlabel('Z-stiffness[N/m]', fontsize=12)
