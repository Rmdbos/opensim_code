...
coor.setValue(state,30/180*np.pi)

# Set all muscle activations to the same and equilibrate muscles
for muscle in model.getMuscles():
    muscle.setActivation(state,0.02)
...
while diff >= 1e-6 and j < 30:
    ...
    print(activation[0].value, activation[1].value)
    # Add one to j
    j += 1
