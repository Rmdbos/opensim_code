# Class takes angle for the simpel model and writes the stationary kinematics file for calculating the H matrix
...
class stat_kine_file:
    ...
    def stat_kine_file(self):
        ...
        file_name_kine = "test_static_kinematics_ID_angle_" + str(round(self.angle,1))

        # Get template file for stationaty kinematics file
        template = open(self.setup_path + r"\templates\Initial_cond_template_test_ID.sto",'r')
        ...
        for i in range(len(time)):
            trace = str(time[i]) + "\t" + str(self.angle/180*np.pi) + "\n"
            file.write(trace)
        file.close()
