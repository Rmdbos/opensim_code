...
def calc_H_test(model,state,activations):
    ...
    dJF = np.zeros((num_coord,num_coord))
    k = 0
    for coordinatei in model.getCoordinateSet():
        ...
        for coordinatej in model.getCoordinateSet():
            ...
            for muscle in model.getMuscles():
                ...
                for p in range(2):
                    if p == 0:
                        angle = coordangle-0.01
                    if p == 1:
                        angle = coordangle+0.01
                    #print(angle)
                    coordinatei.setValue(state,angle)
                    arm = solver.solve(state,coordinatej,path)
                    Mpm[p,0] = coordinatej.getValue(state)
                    Mpm[p,1] = arm
                    g=0
                Mderiv += ((Mpm[1,1] - Mpm[0,1])/0.02)*forces[j]
    ...
    K_j = np.matmul(M,np.matmul(K,Mt)) +dJF
    print(K_j)
